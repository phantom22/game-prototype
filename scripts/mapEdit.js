(function(){
  ...
  D.qSA("#edit",document,0).addEventListener("click",function(){
    ...
    // C = color, d = isDrawing, c1 = centerPoints1, c2 = centerPoins2
    ...
    if (g && g.length == 2 && g[0] <= 101 && g[1] <=101 && g[0] > 0 && g[1] > 0) {
      ...
      // creating a meta string only of zeros (air tiles)
      for (let i=0;i < g[0]*g[1];i++) {meta.push(0)}

      // creating a game
      let instance = new gameInstance({gamemode:3},{grid:g,meta:meta.join("")});

      // creating this var to easily change between class and id for the export function
      let typeId = {"air":0,"wall":1,"coin":2,"door":3};

      // adding an event listener to all the .air (all the tds) of the game table
      D.qSA(".air").forEach(v =>{
        ...
        function draw(e) {
          ...
          if (e && e instanceof HTMLElement) {
            ...
            // getting the coordinates based on the two datasets
            let x = e.dataset.x; let y = e.dataset.y;

            // registering and updating the display of each tile
        ...
        // on mousedown the d becomes true so it's possible to draw with the mouse1 pressed, if the class of the tile is the same as the class of the selected block draw then the function will not proceed
      ...
      // on mouse1 release d becames false
      ...
      D.qSA("#export",document,0).addEventListener("click",function(){
        ...
        // getting each tds class and transforming it into id so it can become and array of tile ids to be then transformed into a string with the join function
      ...
      // if both numbers of the grid are odd then the center of the level will be made of 1 tile
      if (isOdd(g[0]) && isOdd(g[1])) {c1 = [(g[0] - 1) / 2]; c2 = [(g[1] - 1) / 2]}
      // if one of the numbers of the grid is odd and the other one is even then there will be 2 tiles in the center
      else if (isOdd(g[0]) && !isOdd(g[1])) {c1 = [(g[0] - 1) / 2]; c2 = [(g[1])/2,((g[1])/2)-1]}
      else if (!isOdd(g[0]) && isOdd(g[1])) {c1 = [(g[0])/2,((g[0])/2)-1]; c2 = [(g[1] - 1) / 2]}
      //if both the numbers of the grid are even then the center will be made of 4 tiles
      ...
      for (let i=0;i<c1.length;i++) {
        ...
        for (let I=0;I<c2.length;I++) {
          ...
          // encapsulating both the coordinates into an array
          let xy = [c1[i],c2[I]];
